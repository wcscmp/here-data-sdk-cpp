...
NetworkStatistics GetStatistics(const http::NetworkResponse& response) {
  ...
  network_statistics.bytes_downloaded = response.GetBytesDownloaded();
  network_statistics.bytes_uploaded = response.GetBytesUploaded();
...
HttpResponse OlpClient::OlpClientImpl::CallApi(
  ...
  for (int i = 1; i <= retry_settings.max_attempts && !context.IsCancelled() &&
    ...
    accumulated_statistics.Accumulate(response.GetNetworkStatistics());
  }
